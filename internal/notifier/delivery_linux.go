...
// FreedesktopDelivery is the Linux explicit-notify adapter. It talks once to
// the session Notifications service and never starts the click-to-focus daemon
// or falls back to beeep after a possible D-Bus effect.
...
func (d *FreedesktopDelivery) checkAndDeliver(ctx context.Context, r notification.Request, readOnly bool) notification.Receipt {
	out := notification.Receipt{
		...
		Backend:       linuxFreedesktopBackend,
...
func (n *dbusNotifications) Submit(ctx context.Context, r notification.Request) (uint32, error) {
	...
	var id uint32
	err := n.conn.Object("org.freedesktop.Notifications", "/org/freedesktop/Notifications").CallWithContext(
		ctx, "org.freedesktop.Notifications.Notify", 0,
		"agent-notifications", uint32(0), "", r.Content.Title, r.Content.Body,
		[]string{}, hints, expire,
	).Store(&id)
	return id, err
}
