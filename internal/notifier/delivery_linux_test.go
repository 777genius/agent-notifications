...
func linuxDelivery(t *testing.T, session *fakeSession) (*FreedesktopDelivery, *pr3Clock) {
	...
	d.Open = func(context.Context) (sessionNotifications, error) {
		session.opens.Add(1)
		return session, nil
	}
	return d, clock
...
func withFatalBeeep(t *testing.T) {
	...
	beeepNotify = func(string, string, any) error {
		t.Fatal("beeep fallback after D-Bus")
		return nil
	}
...
func TestFreedesktopRequiredNavigationDoesNotOpenSession(t *testing.T) {
	withFatalBeeep(t)
	session := &fakeSession{}
	d, clock := linuxDelivery(t, session)
	req := linuxNoneRequest(clock)
...
func TestFreedesktopReadyFailureDoesNotSubmit(t *testing.T) {
	withFatalBeeep(t)
	session := &fakeSession{ready: errors.New("notifications unavailable")}
	d, clock := linuxDelivery(t, session)
	got := d.Deliver(context.Background(), linuxNoneRequest(clock))
