...
func TestParseCodexPluginListContract(t *testing.T) {
	const expected = "agent-notify@managed"
	status, specs := parseCodexPluginList([]byte(`{"ok":true}`), expected)
	if status != codexListUnknown || len(specs) != 0 {
		t.Fatalf("probe contract: %v %v", status, specs)
	}
	status, specs = parseCodexPluginList([]byte(`{"installed":[]}`), expected)
	...
	status, specs = parseCodexPluginList([]byte(present), expected)
	...
	status, specs = parseCodexPluginList([]byte(foreign), expected)
	if status != codexListAbsent || len(specs) != 0 {
		t.Fatalf("foreign: %v %v", status, specs)
	}
	sameNameForeignMarketplace := `{"installed":[{"pluginId":"agent-notify@unrelated-marketplace","name":"agent-notify","marketplaceName":"unrelated-marketplace","installed":true,"enabled":true}]}`
	status, specs = parseCodexPluginList([]byte(sameNameForeignMarketplace), expected)
	if status != codexListAbsent || len(specs) != 0 {
		t.Fatalf("unrelated namespace: %v %v", status, specs)
	}
}
