...
func Apply(ctx context.Context, o Options, r Request) (result Result, err error) {
	...
	if !setupOSSupported(o.Platform) {
		return result, fail("unsupported_platform", fmt.Errorf("explicit setup supports macOS and Windows"))
	}
...
func (o Options) validatePrepared(ctx context.Context, s installruntime.PolicySnapshot, prepared []byte) error {
	...
	if o.Platform == "windows" && p.Route.LocalRouting {
		return fail("unsupported_platform", fmt.Errorf("windows explicit setup supports navigation none"))
	}
...
func Inspect(ctx context.Context, o Options, r Request, preparedGlobal []byte) error {
	...
	if !setupOSSupported(o.Platform) {
		return fail("unsupported_platform", fmt.Errorf("explicit setup supports macOS and Windows"))
	}
