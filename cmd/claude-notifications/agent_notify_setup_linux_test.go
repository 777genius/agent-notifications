...
func TestLinuxSetupNotificationsEnableNoneWithoutNative(t *testing.T) {
	...
	if _, e := journal.Open(setupCommandContext(t), journal.Options{Root: filepath.Join(control, "state", "journal"), Clock: journal.DefaultClock()}); e != nil {
		t.Fatal(e)
	}
}
