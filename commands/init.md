...
where this installer can configure MCP (macOS and Linux, `navigation=none` on
Linux). Auto-default on another OS keeps hooks-only setup and reports
...
if [ "$SKIP_AGENT_NOTIFY" != true ]; then
  ...
  if [ "$os" != darwin ] && [ "$os" != linux ]; then
    if [ "$SEEN_AGENT_NOTIFY" = true ]; then
      echo "agent-notify MCP is unsupported on this OS; this installer supports macOS and Linux. Plugin install succeeded. Not a full MCP installation." >&2
      exit 1
    fi
    echo "agent-notify MCP skipped: unsupported_platform (this installer supports macOS and Linux). Plugin install succeeded. Not a full MCP installation." >&2
